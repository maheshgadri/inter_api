...
// };
  ...
  router.post('/saveResponses', (req, res) => {
    const { question_text, option1, option2, option3, option4, answer, user_id } = req.body;

    // Check for required fields
    if (!question_text || !option1 || !option2 || !option3 || !option4 || !answer || !user_id) {
        return res.status(400).json({ error: 'question_text, options, answer, and user_id are required' });
    }

    // Insert question, options, and answer into MySQL database
    const query = 'INSERT INTO destination_openai_response (question_text, option1, option2, option3, option4, answer, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)';
    const values = [question_text, option1, option2, option3, option4, answer, user_id];
